<div>
    ...
    <form method="post" action="{{ route('create.comment' , $post) }}" class="p-2 flex items-center justify-start">
        ...
        <a href="{{ route('show.comments' , $post) }}" class="text-gray-800 ms-1 md:ms-12 bg-gray-100 py-1 px-4 rounded-lg cursor-pointer hover:bg-gray-200">{{ __('messages.comments') }}&nbsp ({{ $post->comments()->count() }})</a>
    </form>
